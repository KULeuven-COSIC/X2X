...
#(
    ...
    parameter BOX_SHARES = (N_STAGES - 1) * 3 * (N_SHARES * (N_SHARES - 1) / 2) + 2 * (N_SHARES * (N_SHARES - 1) / 2),
    
    
    parameter RND_SHARES = 2 * B2A_RND_SHARES + 2 * EXPAND_SHARES + 2 * TRIANGLE_SHARES,
    parameter RND_SHARES_BOX = 2 * BOX_SHARES,
    
    // MINIMUM = LSFR_WIDTH * (PARAM_WIDTH + BOX_WIDTH) = 12
    parameter SEED_WIDTH = 12*128 
)
(
    // GENERAL
    input logic clk,
    ...
    output logic [BOX_WIDTH - 1 : 0]    rnd_out_16bit  [RND_SHARES_BOX - 1 : 0],
    ...
    logic [32 - 1 : 0] counter_16bit;
    
    logic [LSFR_WIDTH - 1 : 0] reset_seed_32bit [PARAM_WIDTH - 1 : 0];
    logic [LSFR_WIDTH - 1 : 0] reset_seed_16bit [BOX_WIDTH - 1 : 0];
    logic request_rnd_32bit;
    logic request_rnd_16bit;
    
    logic [PARAM_WIDTH - 1 : 0] rnd_share_32bit;
    logic [BOX_WIDTH - 1 : 0]   rnd_share_16bit;
    
    logic [BOX_WIDTH - 1 : 0]   rnd_reg_16bit   [RND_SHARES_BOX - 1 : 0];
    ...
    localparam STATE_BOX_IDLE     = 2'h0;
    localparam STATE_BOX_A2B      = 2'h1;
    localparam STATE_BOX_WAIT     = 2'h2;
    localparam STATE_BOX_FINISH   = 2'h3;
    
    logic [2 - 1 : 0] state_16bit;
    ...
    assign rnd_out_16bit = (prng_done) ? rnd_reg_16bit : '{default : '0};
    
    generate
    for (idx = 0; idx < PARAM_WIDTH; idx++)
    begin
        assign reset_seed_32bit[idx] = seed_in[idx*LSFR_WIDTH + LSFR_WIDTH - 1 : idx*LSFR_WIDTH];
    end
    
    for (idx = 0; idx < BOX_WIDTH; idx++)
    begin
        assign reset_seed_16bit[idx] = seed_in[idx*LSFR_WIDTH + LSFR_WIDTH + LSFR_WIDTH*PARAM_WIDTH - 1 : idx*LSFR_WIDTH + LSFR_WIDTH*PARAM_WIDTH];  
    end
    endgenerate
        ...
        begin
            case(state)
                ...
                begin
                    if (conversion_type)
                    begin
                        rnd_reg[counter] <= rnd_share_32bit;
                ...
                STATE_B2A_RND :
                begin
                    rnd_reg[counter] <= (mod_type) ? (rnd_share_32bit[13 - 2 : 0] < KYBER_Q) ? {20'b0,rnd_share_32bit[13 - 2 : 0]} : {20'b0, rnd_share_32bit[13 - 2 : 0] - 3329} : rnd_share_32bit;
                ...
                STATE_WAIT : 
                begin 
                    request_rnd_32bit <= 1'b0;
                    
                    if (state_16bit == STATE_BOX_WAIT)
                    begin
                        state <= STATE_FINISH;
                    end
                    else 
    ...
    begin
        ...
        begin
            case(state_16bit)
                ...
                STATE_BOX_A2B :
                begin 
                    rnd_reg_16bit[counter_16bit] <= rnd_share_16bit;
                    counter_16bit <= counter_16bit + 1;
                    
                    if (counter_16bit == BOX_SHARES - 1)
                    begin
                        ...
                        begin
                            state_16bit <= STATE_BOX_WAIT;
                        end
                    end
                    else if (counter_16bit == 2 * BOX_SHARES - 1)
                    begin
                        state_16bit <= STATE_BOX_WAIT;
                    end
                    else 
    ...
    (
        ...
        .rst_n(rst_n),
        .seed_prng(load_seed),
        .request_rnd(request_rnd_32bit),
        .reset_seed(reset_seed_32bit),
        .en_rnd(1'b1),
        .rnd(rnd_share_32bit)
    );
...
endmodule : PRNG_engine_32b_2SHARE
