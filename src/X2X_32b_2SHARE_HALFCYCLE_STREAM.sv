...
module X2X_32b_2SHARE_HALFCYCLE_STREAM
#(
...
(
    ...
    input logic [BOX_WIDTH - 1 : 0]     fresh_rnd_shares_8bit   [RND_SHARES_BOX - 1 : 0], // FOR 2 SECADD, either mod q or power-of-two in parallel
    ...
    logic [BOX_WIDTH - 1 : 0]   fresh_rnd_box       [2 - 1 : 0][RND_BOX_SHARES - 1 : 0];
	...
	logic  data_type_reg;
	logic  dual_mode_reg;
	logic [PARAM_WIDTH - 1 : 0] modulus_reg;
	logic [PARAM_WIDTH - 1 : 0] modulus_twoc_reg;
	...
	generate
        ...
        D_reg_modulus
        (.clk(clk), .rst_n(rst_n), .en(1'b1), .d(modulus), .q(modulus_reg));
        
        (* keep_hierarchy = "TRUE" *) D_reg 
        #(.PARAM_WIDTH(PARAM_WIDTH)) 
        D_reg_modulus_twoc
        (.clk(clk), .rst_n(rst_n), .en(1'b1), .d(modulus_twoc), .q(modulus_twoc_reg));
	...
	begin
	   ...
	   begin
           ...
           begin
                if (i == 0) 
                begin
                    b_SecAdd[1][i] = data_type_reg ? (S_SecAdd_sel[i]) ? modulus_twoc_reg : '0 : expand_reg[1][N_SHARES + i]; //-q is positive, +0 if negative
                end
                else 
                begin
                    b_SecAdd[1][i] = data_type_reg ? done_SecAdd_reg[0] ? (S_SecAdd_sel[i]) ? '0 : modulus_twoc_reg : '0 : expand_reg[1][N_SHARES + 1]; //-q is positive, +0 if negative
                end
            end       
	   ...
	   begin
	       ...
	       for (int i = 0; i < N_SHARES; i++) 
           begin
                b_SecAdd[1][i] = data_type_reg ? (S_SecAdd_sel[i]) ? modulus_reg : '0 : expand_reg[1][N_SHARES + i]; //-q is positive, +0 if negative
           end     
...
//            assign active_fresh_rnd[idy] = en_expand_reg[0] ? fresh_rnd_shares[idy + 2 * N_SHARES] : '0; // HERE
	...
	begin
        ...
        (* keep_hierarchy = "TRUE" *) SecAdd_32b_HALFCYCLE_STREAM
        #(
            .HALFCYCLE(HALFCYCLE),
            .N_SHARES(N_SHARES),
            .PARAM_WIDTH(PARAM_WIDTH)
